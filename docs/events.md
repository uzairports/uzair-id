...
Вызывается при завершении сеанса пользователя: через маршрут `uzair.logout` и через
`uzair.token`, когда middleware отказывает сессии — вход завершили с другого устройства или
его больше нечем обновить. Это две половины одного и того же, и раньше слышна была только
первая.
